...
load([file_prefix '_cluster_feats_resize.mat'], 'imgInfo');

write_dir = '/home/yjlee/web/weakVideo/cluster_matches2/';
...
if exist([write_dir cls '_img' num2str(imNdx(end)) '_match' num2str(num_matches) '.jpg'],'file')
    load([file_prefix '_cluster_matches_resize.mat'], 'match_vals');
    [match_vals, sorted_ndx] = sort(match_vals,2,'descend');
else
    load([file_prefix '_cluster_matches_resize.mat'], 'match_vals','match_ndxs','boxes','frame_ndxs');
    ...
    for imNdx = 1:skip:numel(imgInfo)
        ...
        if box_height>box_width            
            scale = ((MAXPYRAFEATLENGTH-1)*PYRASTRIDE+1)/box_height;
        else
            scale = ((MAXPYRAFEATLENGTH-1)*PYRASTRIDE+1)/box_width;
        end
        im_box = bsxfun(@times, ([1 1 imgInfo(imNdx).feat_size(2) imgInfo(imNdx).feat_size(1)] - 1), PYRASTRIDE/scale);
        im_box = [x1 y1 x1 y1] + im_box;
        ...
        pyra_width = imgInfo(imNdx).feat_size(2);
        pyra_height = imgInfo(imNdx).feat_size(1);
